...
def get_llm():
    if app_config.model.llm_provider == LLMProvider.OPENAI:
    ...
    elif app_config.model.llm_provider == LLMProvider.COHERE:
        return ChatCohere(
        ...
        return ChatGoogleGenerativeAI(
            ...
            google_api_key=app_config.model.api_key
        )
...
def get_embedding_model():
    ...
    return CohereEmbeddings(
        cohere_api_key=app_config.model.embedding_api_key,
        model=app_config.model.embedding_model
    )
...
def retrieve(query: str):
    """
    Perform a similarity search to retrieve relevant information from the vector database.
    """
    logging.info("Using retrieve tool with query: %s", query)
...
def should_query(state: MessagesState):
    ...
    response = chain.invoke(new_messages)
...
async def create_chat_graph():
    """Create and return the compiled graph with checkpointer"""
    ...
    async with get_db_connection() as pool:
        ...
        return graph_builder.compile(checkpointer=checkpointer), checkpointer
...
async def get_graph():
    ...
    if _graph is None:
        _graph, _checkpointer = await create_chat_graph()
    return _graph
